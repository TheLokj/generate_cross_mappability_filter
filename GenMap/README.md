...
- `final_mask.bed`, which is a mask combining `target_unique.bed` with all `overlap_${other_file}.bed`.

## How it works

To be quicker, this version of the script uses GenMap. This tool is designed to compute the uniqueness of k-mers for each position in a genome while allowing for up to e mismatches. It refers, for each position of a genome, the frequency in the index (i.e., in the same file) of the k-mer starting at this position. 

To be able to use this tool to compute a cross-mappability filter, this one is used in its "multi-fasta" behavior. The idea is firstly to launch the tool in its classical way, and referencing the self-mappability of the target. Then, an index is created including the target genome and the *other* fasta, referencing the frequency of each k-mers in both fasta, and these ones are mapped on the target. Thanks to the self-mappability reference, it's possible to deduce from the `(Target+Other)` frequency at each positionn the frequency of the k-mer within the target in order to get the frequency within the *other*. This allows to identify quickly the k-mers from *other* which are able to aligns with the target, and where. 
...
### About stringency and the Sensitivity/Specificity trade-off

The `-r` parameters (`-rs` for self-mappability, `-rc` for cross-mappability) allow you to specify a stringency, *i.e.*, for a base, the number of overlapping k-mers required to be classified as identified. 

The self-mappability stringency is used to filter repetitions in the target. With `-k`=35 and `-rs`=0.5, a base is only kept if at least 18 overlapping `-k`-mers are unique in the target. Note that this stringency will only impact the `target_unique.bed` file, keeping others results safe.

The script allows also a cross-mappability stringency (`-rc`). If `-rc=0.0`, a base is highlighted as problematic if at least 1 overlapping *other* k-mer matches the target genome (highly conservative). If `-rc=0.99`, a base is highlighted only if nearly all overlapping k-mers match the target genome (highly permissive/specific). For example, with `-k`=35 and `-rc`=0.5, a base is highlighted if at least 18 overlapping `-k`-mers are shared between the target and the *other*. This cross-mappability filter **does not** take into account the uniqueness of the *other* k-mer in the target, as GenMap only report frequency. 
...
To make this script quicker, you can increment the number of threads used for mapping `-j`.
